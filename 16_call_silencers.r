...
for (i in 1:nrow(sig_gene_tpms_norm)){
    ...
    assoc_peaks <- sig_peak_covs_norm_inverted %>% filter(parent == gene_id)
...
write.table(all_SS_profiles, "silencer_SS_profiles.tsv", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)