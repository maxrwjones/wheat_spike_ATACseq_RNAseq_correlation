...
peak_results <- read.csv("peaks_UMRs_ImpulseDE2_results.csv", header=TRUE)
...
################################################################################
### Start analysing UMR-supported peaks for significations DE parent genes #####
################################################################################

#### Pull out UMR-supported peaks with significantly DE parent genes
...
### Pull out UMR-supported peaks with non-DE parents
...
### Pull out UMR-supported peaks with non-DE or unexpressed parents
...
################################################################################
### Pull out UMR-supported peaks with unexpressed parents
...
##########################################################################################
### Shuffle UMR-supported peaks' parent genes randomly to generate a null distribution ###
##########################################################################################

### Create a set of empty vectors which will be populated with summary statistics from the peak sets with random parents
AP_SG <- c()    # Vector for "average number of any UMR-supported peaks per sig parent"
SP_SG <- c()    # Vector for "average number of sig UMR-supported peaks per sig parent"

AP_NSG <- c()    # Vector for "average number of any UMR-supported peaks per non-sig parent"
SP_NSG <- c()    # Vector for "average number of sig UMR-supported peaks per non-sig parent"

AP_NSUG <- c()    # Vector for "average number of any UMR-supported peaks per non-sig or unexpressed parent"
SP_NSUG <- c()    # Vector for "average number of sig UMR-supported peaks per non-sig or unexpressed parent"

AP_UG <- c()    # Vector for "average number of any UMR-supported peaks per unexpressed parent"
SP_UG <- c()    # Vector for "average number of sig UMR-supported peaks per unexpressed parent"

first_parents <- c() # Vector to store the newly assigned 'parent' gene of the first UMR-supported peak in each loop for checking purposes


for (i in 1:500){
    
    ### Shuffle which parent genes are associated with which UMR-supported peak (sampling genes with replacement)
    shuffled_parents <- transform(peak_parents, parent = sample(gene_results$gene, replace=TRUE, size=nrow(peak_parents) ) )
    
    ## Store the new 'parent' of the first UMR-supported peak in a vector after each shuffle for checking purposes
    first_parents <- c(first_parents, head(shuffled_parents$parent)[1])
    
    ###############################################################################################                              
                                  
    ### Filter "shuffled_parents" for UMR-supported peaks with significantly DE 'parent' genes
    shuff_peaks_with_sig_parents <- subset(shuffled_parents, parent %in% sig_genes$gene)
    
    ### Filter "shuffled_parents" for UMR-supported peaks with non-DE 'parent' genes
    shuff_peaks_with_nonsig_parents <- subset(shuffled_parents, parent %in% nonsig_genes$gene)
    
    ### Filter "shuffled_parents" for UMR-supported peaks with non-DE or unexpressed 'parent' genes                              
    shuff_peaks_with_nonsig_or_NA_parents <- subset(shuffled_parents, parent %in% nonsig_and_NA_genes$gene)
    
    ### Filter "shuffled_parents" for UMR-supported peaks with unexpressed 'parent' genes                                   
    shuff_peaks_with_NA_parents <- subset(shuffled_parents, parent %in% NA_genes$gene)
                                  
    ###############################################################################################
    
    ### Calculate average number of any UMR-supported peaks per sig parent
    AP_SG <- c(AP_SG, nrow(shuff_peaks_with_sig_parents) / nrow(sig_genes))
    ### Calculate average number of sig UMR-supported peaks per sig parent
    SP_SG <- c(SP_SG, nrow(shuff_peaks_with_sig_parents %>% filter(padj < 0.05)) / nrow(sig_genes))

    
    ### Calculate average number of any UMR-supported peaks per non-sig parent
    AP_NSG <- c(AP_NSG, nrow(shuff_peaks_with_nonsig_parents) / nrow(nonsig_genes))
    ### Calculate average number of sig UMR-supported peaks per non-sig parent
    SP_NSG <- c(SP_NSG, nrow(shuff_peaks_with_nonsig_parents %>% filter(padj < 0.05)) / nrow(nonsig_genes))
                          
                                  
    ### Calculate average number of any UMR-supported peaks per non-sig or unexpressed parent
    AP_NSUG <- c(AP_NSUG, nrow(shuff_peaks_with_nonsig_or_NA_parents) / nrow(nonsig_and_NA_genes))
    ### Calculate average number of sig UMR-supported peaks per non-sig or unexpressed parent
    SP_NSUG <- c(SP_NSUG, nrow(shuff_peaks_with_nonsig_or_NA_parents %>% filter(padj < 0.05)) / nrow(nonsig_and_NA_genes))
                                  
    
    ### Calculate average number of any UMR-supported peaks per unexpressed parent
    AP_UG  <- c(AP_UG, nrow(shuff_peaks_with_NA_parents) / nrow(NA_genes))
    ### Calculate average number of sig UMR-supported peaks per unexpressed parent
...
cat("Check which genes got assigned to be parent of first UMR-supported peak in the first 20 loops - should be randomly selected from all HC, non-ChrUn genes")
head(first_parents, n=20)

cat("Check the first 20 values for average number of any UMR-supported peaks per sig parent")
head(AP_SG, n=20)
cat("Check the first 20 values for average number of sig UMR-supported peaks per sig parent")
head(SP_SG, n=20)

cat("The average number of any UMR-supported peaks per sig parent across 500 shuffled datasets is: ")
cat(mean(AP_SG))
...
cat("The average number of sig UMR-supported peaks per sig parent across 500 shuffled datasets is: ")
cat(mean(SP_SG))
cat("\nAnd the standard deviation of this is: ")
cat(sd(SP_SG))
...
################################################################################
### Plot histogram of numbers of any UMR-supported peaks per sig DE gene #######
################################################################################
...
ggplot() + aes(AP_SG) +
    ...
    labs(title = "Numbers of any UMR-supported CACRs per significantly DE gene in 500 shuffled datasets",
         x = "Number of UMR-supported CACRs",
         y = "Count") +
    theme_classic() +
    ...
    ggsave("UMR_CACRs_per_DEG.svg", height=12, width=50, unit="cm", limitsize = FALSE)

################################################################################
### Plot histogram of numbers of sig UMR-supported peaks per sig DE gene #######
################################################################################
...
ggplot() + aes(SP_SG) +
    ...
    geom_segment(aes(x=mean_plus_5sd, y=0, xend=mean_plus_5sd, yend=50), color="purple", linetype="dashed", size=1) +
    geom_segment(aes(x=mean_minus_5sd, y=0, xend=mean_minus_5sd, yend=50), color="purple", linetype="dashed", size=1) +
    labs(title = "Numbers of significantly changing UMR-supported peaks per significantly DE gene in 500 shuffled datasets",
         x = "Average number of UMR-supported dCACRs per DEG",
...
ggsave("UMR_dCACRs_per_DEG.svg", height=12, width=50, unit="cm", limitsize = FALSE)