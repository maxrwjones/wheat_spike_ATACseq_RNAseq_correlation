...
for (i in 1:nrow(sig_gene_tpms_norm)){
    ...
    assoc_peaks <- sig_peak_covs_norm %>% filter(parent == gene_id)
...
write.table(all_SS_profiles, "enhancer_SS_profiles.tsv", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)