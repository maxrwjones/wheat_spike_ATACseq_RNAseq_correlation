...
sig_peak_covs_norm <- read.table("sig_peak_covs_norm.tsv", header=TRUE)
...
set.seed(array_ID)
sig_peak_covs_shuff <- transform(sig_peak_covs_norm, parent = sample(sig_peak_covs_norm$parent, replace=FALSE))
...
### Export putative enhancers
filename <- paste0("enhancer_simulations/simulated_enhancer_SS_profiles_", array_ID, ".tsv")
